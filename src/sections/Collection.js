...
const Window = ({image,style}) => {
	return(
		<div className={clsx("bg-white rounded-2xl overflow-hidden shadow-[0_5px_60px_-15px_rgba(0,0,0,0.2)] z-10 col-span-10 transition",style)}>
			...
			<div className="sm:h-[410px] h-[160px] overflow-hidden border-4 border-t-0 border-white rounded-bl-2xl rounded-br-2xl relative">
...
const Collecton = ({data}) => {
  return (
    <section className="sm:pt-20 pt-6 pb-40">
      <Container>
          <div className="sm:text-[56px] text-[32px] sm:text-left text-center w-full font-extrabold pb-1">{data.title}</div>
          <div className="text-slate-600 sm:text-lg text-base sm:text-left text-center font-normal mt-9 tracking-wide">{data.desc}</div>
        <Tab.Group className={'sm:pt-20 pt-5'}>
          <div className="flex-1 flex-row sm:grid grid-cols-12 gap-20">
            <Tab.List className={"flex col-span-3 sm:flex-col flex-start gap-4 sm:overflo-hidden overflow-x-scroll sm:mb-0 mb-5"}>
              {
                data.items.map((item,index) => (
                  <Tab className={"outline-none sm:mb-0 mb-1"} key={index}>
                    {({ selected }) => (
                      <div className={clsx('flex text-xl font-medium w-full  p-4 sm:pr-4 pr-6 rounded-xl transition', selected && 'shadow-dark bg-white')}>
                        <div className="flex flex-1 items-center gap-4 color-slate-900 tracking-wide whitespace-nowrap">
                            {item.icon}
                            {item.name}
                          </div>
                          {selected && (<svg className="sm:block hidden" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                data.items.map((item,index) => (
                  <Tab.Panel key={index}>
                    <div className="grid grid-cols-12 relative">
                      <Window image={item.images[0]} style={'sm:w-auto w-[260px]'}/>
                      <Window image={item.images[1]} style={'!absolute sm:left-28 left-8 sm:top-24 top-14 sm:w-auto w-[260px]'}/>
                      <div className={clsx(
                        "sm:h-72 h-[105px] sm:w-64 w-[94px] overflow-hidden border-4 border-white rounded-2xl z-10",
